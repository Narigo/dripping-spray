describe("Spray", function() {
  ...
  it("should start dripping when hitting the threshold and thus draw a line", function() {
    ...
    var i;
    for (i = 0; i < dripThreshold / spraySize + 1; i++) {
      mySpray.draw(DrawerMock, { x: 500, y: 500 });
    }
    expect(DrawerMock.amountOfLines()).toBe(1);
  });

  it("should not draw lines that are bigger than the drip threshold (the amount of spray needed to drip)", function() {
    var i;
    for (i = 0; i <= dripThreshold / spraySize; i++) {
      spray.draw(DrawerMock, { x: 500, y: 500 });
    }
    expect(DrawerMock.maxSizeOfLines() <= dripThreshold);
  });

  it("should draw multiple lines after hitting the drip threshold", function() {
    var i;
    for (i = 0; i <= dripThreshold / spraySize; i++) {
      spray.draw(DrawerMock, { x: 500, y: 500 });
    }
